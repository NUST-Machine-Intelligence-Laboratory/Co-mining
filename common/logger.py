...
class Logger(object):
    ...
    def savefile(self, content, filename):
        assert f'{self.logging_dir}/{filename}' is not None
        print_to_logfile(logfile=f'{self.logging_dir}/{filename}', content=content, init=False)      
    
    def cearte(self, filename):
        f = open(f'{self.logging_dir}/{filename}', 'w')
        f.close()
    ...
    def info(self, content):
        ...
        print_to_logfile(logfile=self.logfile_path, content=content, init=False)
        timestamp = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        print_to_console(f'{PStyle.green}{timestamp}{PStyle.end} - | {PStyle.yellow}INFO{PStyle.end}     | - {PStyle.yellow}{content}{PStyle.end}')
