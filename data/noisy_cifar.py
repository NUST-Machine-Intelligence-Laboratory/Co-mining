...
def noisify_dataset(nb_classes=10, train_labels=None, noise_type=None,
    ...
    if closeset_noise_ratio==0 and openset_noise_ratio ==0:
...
class NoisyCIFAR100(torchvision.datasets.CIFAR100):
    ...
    def __getitem__(self, index):
        if self.noise_type != 'clean':
            img, target, isclean, truth = self.data[index], self.noisy_labels[index], self.noise_or_not[index], self.targets[index]
        else:
            img, target, isclean, truth = self.data[index], self.targets[index], True, self.targets[index]
        ...
        # return {'index': index, 'data': img, 'label': target, 'isclean': isclean, 'groundtruth': groundtruth}
        return img, target, truth
...
class NoisyCIFAR10(torchvision.datasets.CIFAR10):
    ...
    def __getitem__(self, index):
        ...
        # return {'index': index, 'data': img, 'label': target, 'isclean': isclean}
        return img, target, truth
